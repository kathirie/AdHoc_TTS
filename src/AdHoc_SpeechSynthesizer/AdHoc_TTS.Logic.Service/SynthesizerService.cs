...
public class SynthesisService(ITtsModelDao modelDao,
                              ITtsVoiceDao voiceDao,
                              IMessageTemplateDao templateDao,
                              IConfiguration config,
                              SsmlValidator validator) : ISynthesisService
{
    private readonly ITtsModelDao _modelDao = modelDao ?? throw new ArgumentNullException(nameof(modelDao));
    private readonly ITtsVoiceDao _voiceDao = voiceDao ?? throw new ArgumentNullException(nameof(voiceDao));
    private readonly IMessageTemplateDao _templateDao = templateDao ?? throw new ArgumentNullException(nameof(templateDao));
    private readonly IConfiguration _config = config ?? throw new ArgumentNullException(nameof(config));
    private readonly SsmlValidator _validator = validator ?? throw new ArgumentNullException(nameof(validator));
    ...
    // Template → SSML → WAV
    public async Task<byte[]> SynthesizeFromTemplateAsync(SynthesisFromTemplateRequest req)
    {
        // load template
